...
with DAG(
    ...
    SELECT
        ...
        (select count(*) from shared_data.users where signup_date <= (current_date - interval '1 day')::date) AS total_users,
        (select count(*) from shared_data.users where signup_date = (current_date - interval '1 day')::date) AS new_users,
        (select count (*) from shared_data.events where event_date::date = (current_date - interval '1 day')::date) AS total_events,
        (select count (order_id) from shared_data.orders where order_date::date = (current_date - interval '1 day')::date) AS total_orders,
        (select coalesce(sum(amount), 0) from shared_data.orders where status = 'completed' and order_date::date = (current_date - interval '1 day')::date) AS total_revenue,
        (select coalesce(avg(amount), 0) from shared_data.orders where status = 'completed' and order_date::date = (current_date - interval '1 day')::date) AS avg_order_amount
