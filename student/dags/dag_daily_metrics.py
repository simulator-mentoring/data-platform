...
with DAG(
    ...
    calculate_metrics = PostgresOperator(
        ...
        sql="""
            ...
            (SELECT COUNT(user_id) FROM shared_data.users WHERE signup_date::date <= (CURRENT_DATE - INTERVAL '1 day')) AS total_users,
            (SELECT COUNT(user_id) FROM shared_data.users WHERE signup_date::date = (CURRENT_DATE - INTERVAL '1 day')) AS new_users,
            (SELECT COUNT(event_id) FROM shared_data.events WHERE event_date::date = (CURRENT_DATE - INTERVAL '1 day')) AS total_events,
            (SELECT COUNT(order_id) FROM shared_data.orders WHERE order_date::date = (CURRENT_DATE - INTERVAL '1 day')) AS total_orders,
            COALESCE((SELECT SUM(amount) FROM shared_data.orders WHERE status = 'completed' AND order_date::date = (CURRENT_DATE - INTERVAL '1 day')), 0) AS total_revenue,
            COALESCE((SELECT AVG(amount) FROM shared_data.orders WHERE status = 'completed' AND order_date::date = (CURRENT_DATE - INTERVAL '1 day')), 0) AS avg_order_amount
