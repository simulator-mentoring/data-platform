    ...
    sql="""
        ...
        SELECT
            ...
            (SELECT COUNT(*)
             ...
             WHERE signup_date <= (CURRENT_DATE - INTERVAL '1 day')::date) AS total_users,

            (SELECT COUNT(*)
             FROM shared_data.users
             WHERE signup_date = (CURRENT_DATE - INTERVAL '1 day')::date) AS new_users,

            (SELECT COUNT(*)
             FROM shared_data.events
             WHERE event_date::date = (CURRENT_DATE - INTERVAL '1 day')::date) AS total_events,

            (SELECT COUNT(*)
             FROM shared_data.orders
             WHERE order_date::date = (CURRENT_DATE - INTERVAL '1 day')::date) AS total_orders,
